...
if section == "1. Text & Layout":
    st.header("Text and Layout")
    st.markdown(
        textwrap.dedent(
            """
            **`st.write`** is the Swiss-army knife — it renders strings, numbers, lists, dicts, and even DataFrames.

            **`st.markdown`** accepts GitHub-flavored markdown (bold, links, lists).

            **`st.columns`** splits the page horizontally so you can place widgets side by side.
            """
        )
    )
    ...
    with right:
        st.info("st.info / st.success / st.warning / st.error")
        st.success("st.info / st.success / st.warning / st.error")
        st.warning("st.info / st.success / st.warning / st.error")
        st.error("st.info / st.success / st.warning / st.error")
    ...
    st.markdown(
        """
        Widgets return the users current choice / input on the current run.
        """
    )
    ...
    if st.button("Say hello"):
        st.success(f"Hello {name}! You are {mood} and your enthusiam is {slider}.")
